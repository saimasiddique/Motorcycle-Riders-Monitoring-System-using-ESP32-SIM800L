/*
  ...
  @author: saima_siddique_tashfia
*/

//Enter WIFI password
const char* ssid     = "Mr Siddique";   //your network SSID
const char* password = "anamabed@022$";   //your network password

//Connect the Ap account Password  http://192.168.4.1
const char* apssid = "ESP32-CAM";
const char* appassword = "12345678";    //AP password must be 8 characters

#include <WiFi.h>
#include <WiFiClientSecure.h>
#include "esp_camera.h"         /Video function
#include "soc/soc.h"            //Used for unstable power supply without restarting
#include "soc/rtc_cntl_reg.h"   //Used for unstable power supply without restarting
...
String Feedback = "";  //Send back client message
...
//An credible ESP32-CAM Module pin setting
...
void setup() {
  WRITE_PERI_REG(RTC_CNTL_BROWN_OUT_REG, 0);  //Turn off the power when the power is unstable and restart the setting
  ...
  //Specify the static IP on the client side
  //WiFi.config(IPAddress(192, 168, 201, 100), IPAddress(192, 168, 201, 2), IPAddress(255, 255, 255, 0));

  WiFi.begin(ssid, password);    //Execute network connection
  ...
  {
    delay(500);
    if ((StartTime + 10000) < millis()) break;  //Wait for 10 seconds to connect
  }
...
//Customize the homepage management interface of the webpage
  ...
  if (client) { 
    ...
    while (client.connected()) {
      if (client.available()) {
        ...
        getCommand(c);   //Disassemble the characters obtained in the buffer to get the command parameters
...
//Disassemble the command string and place it into the variable
